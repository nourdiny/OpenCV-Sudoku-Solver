...
def intializePredectinoModel():
    model = tf.keras.models.load_model('Resources/myModel.h5')
    return model
def preProcess(img):
...
def displayNumbers(img , numbers , color=(0,255,0)):
    ...
    for x in range(0,9):
        for y in range(0,9):
            if numbers[(y*9)+x] != 0:
                cv2.putText(img, str(numbers[(y*9)+x]), (x*secH+int(secW/2)-10,int((y+0.8)*secH)),
